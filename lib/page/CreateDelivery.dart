...
class CreateDelivery extends StatefulWidget {
  ...
  @override
  _CreateDeliveryState createState() =>
      _CreateDeliveryState(OrderNo, ShopID, ShopName, DistID, DistName);
}
...
class _CreateDeliveryState extends State<CreateDelivery> {
  ...
  _CreateDeliveryState(this._orderNo, this._shopID, this._shopName,
      this._distID, this._distName);
  ...
  Future _CreateDelivery() async {
    ...
    response = await apiCall.deliveryCreate(deliveryNo, deliveryDatetime,
        userName, _orderNo, '111', _shopName, total);

    if (response == 'success') {
      showSuccessMessage(context, 'ສົ່ງສິນຄ້າສຳເລັດ');
    } else {
      showErrorMessage(context, 'ສົ່ງສິນຄ້າບໍ່ສຳເລັດ');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Center(child: Text('ສົ່ງສິນຄ້າ')),
      ),
